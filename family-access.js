...
async function userContext(userId, activeFamilyId, client = db) {
  const userResult = await client.query('SELECT id, email, family_name, account_status, is_superadmin, approved_at, rejection_reason, email_verified_at FROM users WHERE id = $1', [userId]);
